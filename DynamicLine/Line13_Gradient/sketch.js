...
function draw() {
	...
	// 在中心振动的小球
	//drawMovingBallAtSpdAtCtr(0.33,0,1);
	//drawMovingBallAtSpdAtCtr(0.16,0,1);
...
function drawGradient()
{
    for(var i=0;i<LD.length;i++)
    {
        ...
        var x = width * i/LD.length;
        
        var val1 = map(xi,-10,10,0,255)
        var val2 = map(yi,-10,10,0,255);
       
        stroke(val2,val2,val2);
        //print(yi);
...
function drawMovingBallAtSpdAtCtr(speed,tBias,dtOnSpeed)
{
	var t = fract(speed*millis()/5000);
	push();
	stroke(0);
	...
	var tMax = 0.03;
	var ballDiameter = 5;
	var Ctr = new Vector(width/2,height/2);
	for(var dt =0;dt<tMax;dt+=dDt)
	{
		var deltaT = dt + tBias;
		var pt = t+deltaT;
		...
		//color(0,0,0,30/speed)
		drawMovingBallAtPos(
			LD,L,pt,deltaT*ballDiameter/tMax,cr,Ctr);
	}
	drawMovingBallAtPos(
		LD,L,pt,ballDiameter,color(0,0,0,255),Ctr);
	pop();
}
	...
	LD,t,ballSize,cr)
{
	...
	//idt = 0.5;
	var Pos = 
		Vector.lerp(V0,V1,idt);

	ellipse(
		Pos.x,Pos.y,ballSize,ballSize);
}


function drawMovingBallAtPos(
	LD,LBase,t,ballSize,cr,Pos)
{
	var vtId = lerp(0,LD.length-1,t);
	...
	var VBase0 = LBase[id0];
	var VBase1 = LBase[id1];

	var P = Vector.lerp(V0,V1,idt);
	var PBase = 
		Vector.lerp(VBase0,VBase1,idt);
	var PBias = P.subtract(PBase);
	var PDisp = Pos.add(PBias);

	ellipse(PDisp.x,PDisp.y,ballSize,ballSize);
}
