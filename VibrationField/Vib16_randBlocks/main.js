// 主程序
var A0; // 0-order 
var A1; // 1-order

// 细胞阵列的更新
var K; // elastic factors
var R;
var Decay;

var w = 200;
var h = 200;
var dt = 0.05;
var stirPower = 100;
var n0 = 1;

function setup() {
	createCanvas(w,h);
	A0 = CreateCellArray(w,h);
	A1 = CreateCellArray(w,h);
	
	K = CreateCellArray(w,h);
	K = ApplyCells_Value(K,n0);
	R = CreateCellArray(w,h);
	R = ApplyCells_Value(R,n0);

	for(var i=0;i<10;i++)
	{
		var xmin = floor(random(0,150));
		var xmax = floor(random(xmin+5,190));
		var ymin = floor(random(0,150));
		var ymax = floor(random(ymin+5,190));
		K = ApplyValueInRect(
			random(1,10),K,xmin,xmax,ymin,ymax);
		R = ApplyValueInRect(
			random(1,10),R,xmin,xmax,ymin,ymax);
	}
	
	Decay = CreateCellArray(w,h);
	Decay = ApplyCells_Value(Decay,0.9999);
	Decay = ApplyValueToBound(Decay,10,0.95);
}

// 函数draw()：作画阶段
var LastBDispT = 0.0;
function draw() {
	fill(255);// 填充白色
	//rect(-1,-1,width+2,height+2);

	var tNow = millis()/1000;
	var passedT = tNow - LastBDispT;
	if(passedT>0.1)
	{
		drawA_Points(A0,-0.1,0.1);
		LastBDispT = tNow;
	}

	for(var i=0;i<20;i++)
	{
		Step(dt);
	}

	//ResonatorVib(tNow);
}

function mousePressed()
{
	A0[mouseX][mouseY] = stirPower;
}

function keyPressed() {
	print(keyCode);
    if (key === 'A') {
    	ClearA0A1();
    } 
}


